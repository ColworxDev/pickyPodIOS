...
- (CGRect)textRectForBounds:(CGRect)bounds {
    ...
    if(self.placeholder && [self.placeholder.lowercaseString rangeOfString:@"password"].location != NSNotFound){
        return  CGRectMake(4,4 ,
                           bounds.size.width - 35, bounds.size.height );
    }
    else {
        return CGRectMake(4, 4, bounds.size.width, bounds.size.height);
    }
